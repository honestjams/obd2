...
Make the diagnostic and resolution steps specific and actionable, not generic.`;
  ...
  try {
    ...
    // Extract JSON — Claude may wrap in code fences despite instructions
    ...
    res.status(500).json({ error: 'Failed to get diagnosis. Please try again.' });
  }
});
...
app.listen(port, () => {
  console.log(`OBD2 Diagnostic App running at http://localhost:${port}`);
});
